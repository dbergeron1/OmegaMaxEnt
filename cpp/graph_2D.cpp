...
//char graph_2D::save_figure_command[100];
...
//	fig_name[0]='\0';
	...
	{
		case PYPLOT:
			strcpy(figs_dir,"figs_data");
			strcpy(file_name_format,"figs_%d.py");
		...
		//	strcpy(save_figure_command,"pyplot.save_fig(");
			break;
		case GNUPLOT:
...
void graph_2D::curve_plot(double *x, double *y, int size_data, const char *extra_commands, int fig_ind_p)
{
	add_data(x, y, size_data);
	curve_plot(extra_commands, fig_ind_p);
}

void graph_2D::curve_plot(const char *extra_commands, int fig_ind_p)
{
	switch (plotting_program)
	{
		case PYPLOT:
...
void graph_2D::plot_with_pyplot(const char *extra_commands, int fig_ind_p)
{
	fstream data_file;
	char tmp_file_name[100];
//	char file_name[200];
...
void graph_2D::plot_with_gnuplot(const char *extra_commands, int fig_ind_p)
{
	fstream data_file;
	char tmp_file_name[100];
	char plot_command[10000];
